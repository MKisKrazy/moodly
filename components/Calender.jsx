            ...
            return(
              <div className='grid grid-cols-7 gap-1' key={rowIndex}>
                  {dayList.map((dayOfWeek,dayOfWeekIndex)=>{
                      ...
                      return(
                        <div style={{background:color}} className={' text-xs sm:text-sm border border-solid p-2 flex items-center gap-2 justify-between rounded-lg ' + ( isToday ? ' text-indigo-500 border-indigo-500 ' : ' text-indigo-100 ' ) + (color === 'white' ? ' text-indigo-400 ' : ' text-white')} key={dayOfWeekIndex}>
                            <p>{dayIndex}</p>
                        </div>
                      )
                  })}
